...
**Trigger 2 · Slash command `/wf-refresh`**. When invoked, the agent runs eight phases (builder-driven, orchestrated by `wf-init/lib/refresher.md`, using `wf-init/lib/state-helpers.sh` for state access). The command bootstrap first extracts and runs refresher.md's **Setup: create helper library** section so `${WF_DIR}/lib/refresh-lib.sh` exists before any phase sources it, and its `_extract_phase` helper is **placeholder-safe**: awk programs use a getline loop in BEGIN (plus brace-form shell positionals) instead of any bare dollar+digits tokens, because OpenCode expands those tokens in command bodies as positional placeholders (`placeholderRegex /\$(\d+)/g` in `session/prompt.ts`), which used to inject `"undefined"`/empty strings and break extraction on every phase. The block-coverage counter also tolerates `grep -c`'s exit status 1 when a single-block phase emits no markers (the assignment would otherwise propagate that status into any caller not wrapped in `||`, killing the run under `set -e`). **Resume/pending state lives OUTSIDE `${WF_DIR}`**: the bootstrap exports `WF_STATE_DIR="${TMPDIR:-/tmp}/wf-refresh-state-<cksum-of-pwd>"` and both `.wizard-resume-phase` and `.wizard-pending-prompts` live there (plus the Phase R5 full review file), because the bootstrap `rm -rf`s `${WF_DIR}` at the start of every run — a documented re-run before resuming used to destroy pending resume state (field report M3):
