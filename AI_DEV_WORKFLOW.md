...
- **Phase R-1** (Global): Fetch wizard version from GitHub; skip refresh if already up-to-date. If a global update is applied (install.sh succeeds), refresh STOPS and asks for a new session + re-run, so the updated wizard drives the refresh.
- **Phase R0** (Validate): Validate `.wizard-state.json` structure and detect active IDEs
- **Phase R1** (Discover): Re-discover project (stack, node engine, commands from `package.json`, etc.) and detect drift — writes accepted drift to the STAGING state, not root
...
- **Phase R6**: Apply approved changes only; on approval update the STAGING state's `build_plan` (promoted to root by the final copy), write `.wizard-managed-files.json`, and commit via an explicit pathspec (the user's other staged work is never unstaged); a fully declined refresh writes nothing
...
- **Phase R-1 · Global version check**: Fetches wizard version from remote; exits early if already up-to-date. When a global update is applied, it stops with a "new session + re-run" message so old and new wizard versions never mix in one session.
...
- **Phase R6 · Apply and close**: Applies approved changes, updates the STAGING state's `build_plan` (the final `cp` promotes the complete staging state to root), writes `.wizard-managed-files.json`, and commits. Bookkeeping (state, `.wizard-managed-files.json`, `.gitignore`) only runs when at least one category is approved; a declined refresh writes nothing. The commit uses an explicit pathspec (`--pathspec-from-file`, supported by `git commit`/`git add`) so it contains only approved paths and never unstages unrelated user work (`git reset --mixed` is not used); the empty-diff guard reads the approved paths into positional arguments (`git diff --cached --quiet -- <paths>`), since `git diff` does not support `--pathspec-from-file`. The staged `wizard-state.json` copy is excluded from the diff plan so it can never be committed to the repo. When Windsurf/Devin is an active IDE and the "Gentle AI — Legacy Path Bridge" rule is missing, R6 reinserts it into the STAGED `AGENTS.md` (same safety net as Phase 8) wrapped in `<!-- WF: DO NOT REGENERATE -->` markers so future refreshes preserve it — and only when `AGENTS.md` is itself approved (added/updated), so a declined refresh writes nothing.
