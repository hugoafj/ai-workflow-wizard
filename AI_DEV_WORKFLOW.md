...
- **Phase R1** (Discover): Re-discover project (stack, node engine, commands from `package.json`, etc.) and detect drift — writes accepted drift to the STAGING state, not root. Also backfills richer AGENTS.md sections (Commands, Code Style, Project Structure, MCPs table) into flat staging discovery fields so a refresh does not flatten the project's own documentation.
...
- **Phase R2 · State/schema migration**: Migrates `.wizard-state.json` to current schema; runs an unconditional, idempotent legacy normalization (`testing.layers` dict → array, flat `discovery.stack_key` → nested `discovery.stack.stack_key`); asks about new optional features.
...
- **Phase R4 · Hash-based diff**: Compares the R3 baseline snapshot (not the live `build_plan`, which the Builder already overwrote) against staging; classifies files as add/update/delete/unchanged, with `deleted_modified` flagged when the user edited a file since the last refresh. An explicit `DEPRECATED_PATHS` list (global-only command artifacts: `wf-cicd`, `wf-cleanup`, `wf-refresh`, `wf-init`, and the orphan `wf-sdd-config` skill/protocol, per IDE plus their skills/protocols) is deleted even when absent from the baseline, because those files are installed globally by `install.sh` and never belong in a project. Deletion paths are deduplicated (`unique_by(.path)`) so a path found both in the baseline and in `DEPRECATED_PATHS` is counted once. When `AGENTS.md` is classified as updated, R4 warns that the regenerated file comes from `state.discovery` and may look flat if discovery fields were never populated (R1's backfill mitigates this).
- **Phase R5 · Review gate**: Shows a real content preview (added → staged content; updated → `diff -u` against staging; deleted/deleted_modified → current content), bounded by `MAX_PREVIEW_LINES` (default 120) per file, then collects user approvals. The final gate offers three choices: apply+commit (default), apply without commit, or cancel (cancel removes the staging dir, the plan, and the baseline — a cleanup that previously only happened inside R6's trap). In non-tty/agent contexts the choice comes from `WF_REFRESH_APPLY_MODE=commit|apply-only|cancel`; anything else aborts instead of guessing.
- **Phase R6 · Apply and close**: Applies approved changes, updates the STAGING state's `build_plan` (the final `cp` promotes the complete staging state to root), writes `.wizard-managed-files.json`, and commits. Bookkeeping (state, `.wizard-managed-files.json`, `.gitignore`) only runs when at least one category is approved; a declined refresh writes nothing. In **apply-only mode** (chosen at the R5 gate), files are copied to the working tree, the manifest and state are still written, and the `apply_only` flag is dropped before promoting — but no `git add`/`git commit` runs, leaving the changes unstaged for the user to review and commit. The commit uses an explicit pathspec (`--pathspec-from-file`, supported by `git commit`/`git add`) so it contains only approved paths and never unstages unrelated user work (`git reset --mixed` is not used); the empty-diff guard reads the approved paths into positional arguments (`git diff --cached --quiet -- <paths>`), since `git diff` does not support `--pathspec-from-file`. The staged `wizard-state.json` copy is excluded from the diff plan so it can never be committed to the repo. When Windsurf/Devin is an active IDE and the "Gentle AI — Legacy Path Bridge" rule is missing, R6 reinserts it into the STAGED `AGENTS.md` (same safety net as Phase 8) as plain content without the `<!-- WF: DO NOT REGENERATE -->` markers, matching how `temp-files/AGENTS.md` carries it — and only when `AGENTS.md` is itself approved (added/updated), so a declined refresh writes nothing.
