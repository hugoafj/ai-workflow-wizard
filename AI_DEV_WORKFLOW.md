...
- **Phase R5 · Review gate**: Shows a real content preview (added → staged content; updated → `diff -u` against staging; deleted/deleted_modified → current content), bounded by `MAX_PREVIEW_LINES` (default 120) per file, then collects user approvals. **Dedicated warning block lists `updated` files with `local_modified: true` and asks a separate approval: `Overwrite locally-modified files?` (stored as `build_plan.approval.overwrite_local`).** The final gate offers three choices: apply+commit (default), apply without commit, or cancel. **In non-tty/agent contexts, if any prompt lacks an answer, R5 emits a structured manifest `GENTLE_AI_WF_REFRESH_NEEDS="prompt=<p1>|prompt=<p2>|...|apply_mode=..."` and exits with code 3 (not 2), leaving staging/plan intact. `WF_REFRESH_RESUME=1` skips R-1..R4 (validates staging/plan exist) and re-enters R5 with the preserved state.** In non-tty contexts the choice comes from `WF_REFRESH_APPLY_MODE=commit|apply-only|cancel`; anything else aborts instead of guessing.
