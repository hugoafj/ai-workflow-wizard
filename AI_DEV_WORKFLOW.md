...
- **Phase R5**: Show grouped diff and collect user approvals; offers apply+commit, apply without commit, or cancel (cancel cleans the staging directory)
...
- **Phase R5 · Review gate**: Shows a real content preview (added → staged content; updated → `diff -u` against staging; deleted/deleted_modified → current content), bounded by `MAX_PREVIEW_LINES` (default 120) per file, then collects user approvals. **Dedicated warning block lists `updated` files with `local_modified: true` and asks a separate approval: `Overwrite locally-modified files?` (stored as `build_plan.approval.overwrite_local`).** The final gate offers three choices: apply+commit (default), apply without commit, or cancel. **In non-tty/agent contexts, if any prompt lacks an answer, R5 emits a structured manifest `GENTLE_AI_WF_REFRESH_NEEDS="prompt=<p1>|prompt=<p2>|...|apply_mode=..."` and exits with code 3 (not 2), leaving staging/plan intact. The restart-semantics `Update global commands?` prompt never reaches this manifest — R-1 resolves it (or cuts the run) before any refresh work.** `WF_REFRESH_RESUME=1` re-enters via the `.wizard-resume-phase` marker: pre-R5 stops (R1/R2 fail-fast prompts) resume at exactly the marked phase — earlier phases skip themselves, the marked phase consumes `WF_REFRESH_ANSWERS` and re-runs fully — and the pipeline flows through R5 normally; runs stopped at R5 (no marker) keep the legacy behavior of skipping R-1..R4 (validates staging/plan exist) and re-entering R5 with the preserved state.** In non-tty contexts the choice comes from `WF_REFRESH_APPLY_MODE=commit|apply-only|cancel`; anything else aborts instead of guessing.
