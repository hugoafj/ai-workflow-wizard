...
- **Phase R4**: Before diffing, a **placeholder gate** fails the phase loudly if any wizard-owned template namespace (`{{answers.*}}`, `{{discovery.*}}`, `{{features.*}}`, `{{version}}`, `{{sdd.backend}}`, `{{PROTOCOL_BODY:*}}`, etc.) appears unresolved in staging — arbitrary `{{ }}` text quoted in project docs is NOT flagged. Then compares the R3 baseline snapshot with staging using SHA256 hashes; classify files as add/update/delete/unchanged (the staged `wizard-state.json` copy is never part of the plan)
