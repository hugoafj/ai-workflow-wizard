...
> (Claude, Cursor, Windsurf `global_workflows/`, Kiro `steering/`, Codex, OpenCode) and as
> SKILL.md in each detected IDE's global skills path (`.claude/skills/`, `.cursor/skills/`,
> `.codex/skills/`, `.config/opencode/skills/`, Windsurf, Devin, Copilot, Antigravity) plus
> the universal `~/.agents/skills/` (1:1 command ↔ skill). Copilot and Antigravity receive
> only the SKILL.md global skills, not global slash commands.
